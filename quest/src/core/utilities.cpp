...
#pragma omp parallel for simd reduction(&& : isUnitary)
    for (qindex i = 0; i < dim; i++)
        isUnitary = isUnitary && isApprox(std::abs(diags[i]), 1, eps);
...
#pragma omp parallel for shared(isHermitian)
    for (qindex idx = 0; idx < (dim * (dim - 1)) / 2; idx++)
    {
        if (!isHermitian)
            continue;

        // Compute r and c from the linear index
        qindex r = static_cast<qindex>(std::ceil((std::sqrt(8 * idx + 1) - 1) / 2));
        qindex c = idx - (r * (r - 1)) / 2;
...
/// consider multithreading or GPU-accelerating this
/// when caller is big and e.g. has GPU memory
#pragma omp parallel for shared(allNonZero)
    for (qindex i = 0; i < dim; i++)
    {
        if (!allNonZero)
            continue;
...
#pragma omp parallel for simd reduction(&& : allNonNeg)
    for (qindex i = 0; i < dim; i++)
        allNonNeg = allNonNeg && (std::real(diags[i]) >= 0);
...
// zero initialize the superoperator in parallel
...
// distribute work across superop[r][c] entries
#pragma omp parallel for collapse(2)
    for (qindex r = 0; r < superopDim; r++)
    {
        for (qindex c = 0; c < superopDim; c++)
        {
            qindex i = r / matrixDim;
            qindex k = r % matrixDim;
            qindex j = c / matrixDim;
            qindex l = c % matrixDim;

            qcomp val = 0;
            for (int n = 0; n < numMatrices; n++)
            {
                auto matrix = matrices[n];
                val += std::conj(matrix[i][j]) * matrix[k][l];
            }
            superop[r][c] = val;
        }
    }
